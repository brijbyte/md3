...
- MDX pages live in their route dir (`src/app/(docs)/<section>/<page>/page.mdx`, demos and
  ...
  them to JSX style objects itself and silently drops object values. Shiki highlights with
  a single generated theme (`satteri/shiki.mjs`, shared with the demo loader): the two
  github themes are zipped rule-by-rule into `var(--shiki-*)` colors, and the
  `classTokens` transformer converts each token's inline style to `sk-*` classes —
  classes and `light-dark()` var values live in `src/components/shiki-theme.css`
  (generated by `scripts/build-shiki-theme.mjs` from next.config.ts, gitignored);
  the pre's inline style is stripped (bg/fg come from docs CSS). Gotcha: satteri
...
- Styled with Tailwind v4 (`@tailwindcss/postcss`); doubles as the Tailwind-integration
  ...
  files beside their components (`NavigationProgress.css`, `SidebarNav.css`, `shiki-theme.css` (generated),
