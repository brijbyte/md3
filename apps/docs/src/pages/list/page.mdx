...
and ripple, and shows the focus ring. `selected` tints the row with the
...
`component="label"` so a click anywhere on the row activates that control: the row becomes a
