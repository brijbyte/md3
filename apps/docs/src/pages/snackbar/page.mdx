...
and react — avoid a delay so short the action becomes impractical to use, and reach for
`closable` instead of a long timeout when a message needs to stay put until the user dismisses it
explicitly.
